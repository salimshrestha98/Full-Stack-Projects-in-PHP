...
<div class="row" id="top-row">
    <div class="col-md-4">
        <div class="row" id="top-picks">
            ...
            <ol>
            @foreach($ts as $s)
                <li class="text-sm"><a href="{{ url('/') }}/company/{{ $s }}" target="_blank" title="{{ $stocks[$s]['name'] }}" class="">{{ $s }}</a></li>
            @endforeach
            </ol>
            </div>
            @endif           
...
<table class="table table-striped table-bordered mb-5" style="width:100%" id="f-list">
    ...
    @foreach($latest_stocks as $st)
        <tr class="@if($stocks[$st->symbol]['index_1'] < 1) bg-danger @endif">
            ...
            <td><a href="{{ url('/') }}/company/{{ $st->symbol }}" target="_blank" title="Company Overview" class="">{{ $st->symbol }}</a></td>
